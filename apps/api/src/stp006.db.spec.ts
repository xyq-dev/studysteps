...
describe.skipIf(shouldSkipStp004Isolation())('STP 006 seventh migration and constraints', () => {
  ...
  it('applies twelve migrations and does not publish policy in the structure migration', async () => {
    ...
    const eleventh = await prisma.$queryRaw<Array<{ migration_name: string }>>`
      SELECT migration_name FROM _prisma_migrations
       WHERE migration_name = '20260919120000_stp006_task_horizon_jobs'
    `;
  ...
  it('rejects duplicate occurrence keys and assisted plans without attestation', async () => {
    ...
    const series = await prisma.taskSeries.create({
      data: {
        ...
        effectiveToLocalDate: '2026-09-17',
      },
    });
    ...
    await expect(
      prisma.taskOccurrence.create({
        data: { ...occ, occurrenceKey: '2026-09-18', originalLocalDate: '2026-09-18' },
      }),
    ).rejects.toThrow();
