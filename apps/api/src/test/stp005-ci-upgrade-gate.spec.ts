...
type UpgradeGate = {
  ...
  assertOriginalIdentityPresence: (rowCount: number, env?: NodeJS.ProcessEnv) => void;
...
describe('STP 005 CI upgrade fixture gate', () => {
  ...
  beforeAll(async () => {
    const specifier = '../../../../scripts/stp005-four-to-six-upgrade.mjs';
  ...
  it('requires the local original database only outside CI/prepare', () => {
    ...
    expect(gate.shouldRequireLocalOriginalIdentity({ CI: '1' })).toBe(false);
    expect(gate.shouldRequireLocalOriginalIdentity({ STP004_CI_PREPARED: '1' })).toBe(false);
    expect(gate.shouldRequireLocalOriginalIdentity({ STP004_CI_PREPARE: '1' })).toBe(false);
    expect(() => gate.assertOriginalIdentityPresence(0, { CI: '' })).toThrow(
      /original stp004_identity missing; leave it read-only/,
    );
    expect(() => gate.assertOriginalIdentityPresence(1, { CI: '' })).not.toThrow();
    expect(() => gate.assertOriginalIdentityPresence(0, { CI: 'true' })).not.toThrow();
    expect(() => gate.assertOriginalIdentityPresence(0, { STP004_CI_PREPARED: '1' })).not.toThrow();
  });
  ...
  it('keeps forbidden and exclusive fixture checks under CI', () => {
    ...
    expect(() => gate.assertAllowed('stp004_identity_fresh')).toThrow(/refusing database/);
    ...
    expect(gate.rewriteDb(dummyAdmin, gate.FIXTURE_DATABASE)).toMatch(/\/stp005_rev_four_to_six$/);
  });
  ...
  it('records the fixture URL on the current env and GITHUB_ENV without relying on later steps', () => {
    ...
    gate.recordFourToSixFixtureUrl(fixtureUrl, env);
    ...
    expect(() =>
      gate.recordFourToSixFixtureUrl(
