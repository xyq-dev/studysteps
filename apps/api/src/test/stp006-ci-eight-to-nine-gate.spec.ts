...
describe('STP 006 CI eight-to-nine upgrade fixture gate', () => {
  const dirs: string[] = [];
  let gate: UpgradeGate;

  beforeAll(async () => {
    const specifier = '../../../../scripts/stp006-eight-to-nine.mjs';
  ...
  it('records the fixture URL on the current env and GITHUB_ENV without relying on later steps', () => {
    const dir = mkdtempSync(join(tmpdir(), 'stp006-9-gh-env-'));
    dirs.push(dir);
    ...
    gate.recordEightToNineFixtureUrl(fixtureUrl, env);
    expect(env.STP006_EIGHT_TO_NINE_DATABASE_URL).toBe(fixtureUrl);
    const text = readFileSync(githubEnv, 'utf8');
    expect(text).toContain('EXISTING=1');
    expect(text).toContain(`STP006_EIGHT_TO_NINE_DATABASE_URL=${fixtureUrl}`);
    expect(() =>
      gate.recordEightToNineFixtureUrl(
        'postgresql://stp004_admin:secret@127.0.0.1:5432/stp006_seven_to_eight',
        env,
      ),
  ...
  it('does not clobber a different local runtime cluster', () => {
    ...
    const ciFixture = 'postgresql://stp004_admin:secret@127.0.0.1:5432/stp006_eight_to_nine';
    const sameCluster = 'postgresql://stp004_admin:secret@127.0.0.1:6260/stp006_eight_to_nine';
