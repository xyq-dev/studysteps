...
test('STP 006 Chromium edits one occurrence and keeps the sibling after refresh', async ({ page }) => {
  const nickname = `本次${Date.now().toString().slice(-6)}`;
  ...
  const originalFirst = await first.innerText();
  const originalSecond = await second.innerText();
  const originalDate = originalFirst.slice(0, 10);
  const originalName = originalFirst.split(' · ')[1] ?? '';
  const siblingName = originalSecond.split(' · ')[1] ?? '';
  await first.getByRole('button', { name: '编辑本次' }).click();
  await expect(page.getByTestId('edit-occurrence')).toBeVisible();
  await expect(page.getByTestId('edit-occurrence-scope')).toContainText('仅影响本次任务');
  await page.getByTestId('cancel-edit-occurrence').click();
  await expect(page.getByTestId('edit-occurrence')).toHaveCount(0);
  await expect(page.getByTestId('task-list')).toContainText(`${originalDate} · ${originalName}`);
  await first.getByRole('button', { name: '编辑本次' }).click();
  await page.getByTestId('edit-occurrence-name').fill('只改这一次');
  await page.getByTestId('edit-occurrence-standard').fill('读完指定页并口头复述');
  ...
  await expect(page.getByTestId('edit-occurrence')).toHaveCount(0);
  await expect(page.getByTestId('task-list')).toContainText(`${originalDate} · 只改这一次`);
  await expect(page.getByTestId('task-list')).toContainText(siblingName);
  await expect(page.getByTestId('task-list')).not.toContainText(`${originalDate} · ${originalName}`);
  await shot(page, '07-s05-after-edit.png');
  await page.reload();
