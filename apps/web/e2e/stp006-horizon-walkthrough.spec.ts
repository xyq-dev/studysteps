...
test('STP 006 Chromium clicks task horizon and refresh stays consistent', async ({ page }) => {
  const nickname = `补齐${Date.now().toString().slice(-6)}`;
  ...
  await expect(page.getByTestId('status')).toContainText('当前窗口无需补齐', { timeout: 15_000 });
  await expect(page.getByRole('heading', { name: 'S05 今日任务' })).toBeVisible();
  const count = await page.getByTestId('task-list').locator('li').count();
  expect(count).toBeGreaterThan(0);
  await shot(page, '05-s05-after-horizon.png');
  await page.getByTestId('update-task-horizon').click();
  await expect(page.getByTestId('status')).toContainText('当前窗口无需补齐');
  await expect(page.getByTestId('task-list').locator('li')).toHaveCount(count);
  await page.reload();
