...
| 6 | STP 006 计划与任务生成 | M2 | STP 004、STP 005 | 重复、编辑范围、改期和并发约束通过。当前**第一批＋S06 最小闭环已实施**（整个阶段未完成），见 `docs/STP006_DESIGN.md` 与 `docs/STP006_IMPLEMENTATION_REPORT.md` |
...
状态：第一批＋S06 最小闭环已实施（整个阶段未完成）。设计见 `docs/STP006_DESIGN.md`；证据见 `docs/STP006_IMPLEMENTATION_REPORT.md`。

2026-09-17：第二批落地 S06 空白创建：`POST /v1/students/:id/plans/preview`（零 INSERT）与 `POST /v1/students/:id/plans`（确认事务复用 14 天窗口）。不伪造模板 ID；`source_template_version_id` 为空。自定义年级无映射仍禁止模板导入，手动创建按教育／同意／`PLAN_CREATE` 判断。七条已发布迁移 checksum 未改，无第八条。范围编辑、暂停／归档、改期、拆分与 Outbox 工人仍属后续批次。STP 004 仍进行中；STP 005 产品验收未完成。GitHub Actions 在 push 后按新 SHA 跟踪。
