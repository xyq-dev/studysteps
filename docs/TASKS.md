...
| 6 | STP 006 计划与任务生成 | M2 | STP 004、STP 005 | 重复、编辑范围、改期和并发约束通过。当前**第一批＋S06＋暂停／恢复／归档已实施**（整个阶段未完成），见 `docs/STP006_DESIGN.md` 与 `docs/STP006_IMPLEMENTATION_REPORT.md` |
...
- 暂停、恢复、归档已按 §3.5 落地（保留历史，归档不可恢复）。范围编辑、改期、拆分仍待后续批次；拆分后的原任务不会再次计为完成。
