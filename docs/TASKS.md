...
| 5 | STP 005 学段与模板种子 | 基础内容 | STP 003；与 STP 004 共享的档案字段已稳定 | 学制映射和 36 个模板可查询、预览 |
...
2026-09-15 续作证据见 `docs/STP004_IMPLEMENTATION_REPORT.md` 第 10 节；2026-09-16 归档与分册登记见第 11 节。**不得** 写“第 1–8 项均修复”，也 **不得** 写“本阶段应执行项均已通过”。四项修复（父键 CHECK／双方反环锁、advisory 后重查与并发 201+429、step-up／logout CSRF 且 SIGN_IN 保持未登录、锁后时钟与四条 HB-2）及第四条迁移 `20260915160000` 在 fresh／two-mig **复核通过**。2026-09-16 未改代码、未重跑。TIME-2 范围仍仅为 idle／absolute／节流刷新。T02-B 可单独通过。T11-3、CON-3 为部分通过（学习写入未实现）。下列三项分册登记，不混作本轮四项或第四条阻塞：原库 `stp004_identity` 污染且只读未升级；T02-D → STP 005、T11-D → 后续阶段、学习写入既定延期；合同层 pointer 与 E2E 非并发等非阻塞补强。入仓库回归与 `.local` 临时探针分开。STP 004 整体仍不得标为完成。环境通过不能代替业务验收。
...
目标：建立可配置的学段、学制、年级、学期和科目映射，并提供 36 个可预览的原创基础模板。
...
- 提供 12 个常见年级入口，每个 3 个原创模板，共 36 个；模板注明适用范围和可调整内容。
