...
| 6 | STP 006 计划与任务生成 | M2 | STP 004、STP 005 | 重复、编辑范围、改期和并发约束通过。当前**第一批＋S06＋暂停／恢复／归档＋按需 horizon＋单次改期已实施**（整个阶段未完成），见 `docs/STP006_DESIGN.md` 与 `docs/STP006_IMPLEMENTATION_REPORT.md` |
...
状态：第一批＋S06＋暂停／恢复／归档＋按需 task-horizon＋单次改期已实施（整个阶段未完成）。设计见 `docs/STP006_DESIGN.md`；证据见 `docs/STP006_IMPLEMENTATION_REPORT.md`。

2026-09-18：本批落地仅本次任务改期。只改 `scheduledLocalDate`，保持 id／key／快照；撞日不覆盖。RESUME 改为恢复今日及之后所有 `PLAN_PAUSED` 行（支持改到 14 天外后的暂停恢复）。第八条只加实例 `version`。七条已发布迁移与 test-v2 不变；`stp006_six_to_seven` 保持七条。范围编辑、拆分、horizon 工人仍属后续。STP 004 仍进行中；STP 005 产品验收未完成。GitHub Actions 在 push 后按新 SHA 跟踪。
...
- 暂停、恢复、归档已按 §3.5 落地（保留历史，归档不可恢复）。单次改期已按 §3.7 落地。范围编辑、拆分仍待后续批次；拆分后的原任务不会再次计为完成。
