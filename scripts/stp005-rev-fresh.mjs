...
const freshName = 'stp005_rev_fresh';
const originalName = 'stp004_identity';
if (FORBIDDEN.has(freshName)) {
  throw new Error('refusing to use a forbidden database name');
}
...
for (const url of [adminUrl, apiUrl]) {
  const name = databaseName(url);
  if (name === freshName) continue;
}
...
if (existsSync(runtimeFile)) {
  ...
  setLine('STP005_REV_FRESH_DATABASE_URL', freshUrl);
  writeFileSync(runtimeFile, text);
  process.stdout.write('runtime.env now points tests at stp005_rev_fresh; original and old-fresh unchanged.\n');
}
