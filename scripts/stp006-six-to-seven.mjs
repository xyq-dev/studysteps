...
const SEVENTH = '20260917120000_stp006_study_plans_occurrences';
...
export const FIXTURE_DATABASE = 'stp006_six_to_seven';
...
export function recordSixToSevenFixtureUrl(fixtureUrl, env = process.env, io = { appendFileSync }) {
  const name = databaseName(fixtureUrl);
  assertAllowed(name);
  env.STP006_SIX_TO_SEVEN_DATABASE_URL = fixtureUrl;
  if (env.GITHUB_ENV) {
    io.appendFileSync(env.GITHUB_ENV, `STP006_SIX_TO_SEVEN_DATABASE_URL=${fixtureUrl}\n`);
  }
}
...
if (isDirectRun()) {
  ...
  for (const folder of SIX) {
    await applySql(fixture, folder);
  }
  const beforeGrades = await fixture.query('SELECT COUNT(*)::int AS n FROM grade_configs');
  const beforeTemplates = await fixture.query('SELECT COUNT(*)::int AS n FROM plan_template_versions');
  ...
  await fixture.query(
    `INSERT INTO student_profiles (
       ...
       $1, 'ONBOARDING', '六到七基线', 'avatar-03', 'UNDER_14', 'GUARDIAN_DECLARATION', clock_timestamp(),
  ...
  const afterSix = new pg.Client({ connectionString: fixtureUrl, connectionTimeoutMillis: 8000 });
  await afterSix.connect();
  await applySql(afterSix, SEVENTH);
  const afterGrades = await afterSix.query('SELECT COUNT(*)::int AS n FROM grade_configs');
  const afterTemplates = await afterSix.query('SELECT COUNT(*)::int AS n FROM plan_template_versions');
  const kept = await afterSix.query(`SELECT nickname FROM student_profiles WHERE id = $1`, [studentId]);
  if (afterGrades.rows[0].n !== beforeGrades.rows[0].n || afterTemplates.rows[0].n !== beforeTemplates.rows[0].n) {
    throw new Error('sixth-to-seventh changed catalog counts');
  }
  if (kept.rows[0]?.nickname !== '六到七基线') {
    throw new Error('baseline student was not preserved');
  }
  const planId = randomUUID();
  const seriesId = randomUUID();
  await afterSix.query(
  ...
  await afterSix.query(
    `INSERT INTO task_series (
     ...
     ) VALUES ($1, $2, '去重', '自定义', '完成', 'ONCE', '2026-09-17', '2026-09-17', false, '2026-09-17', 1, clock_timestamp(), clock_timestamp())`,
    [seriesId, planId],
  );
  const occSql = `INSERT INTO task_occurrences (
    ...
    grade_code_snapshot, grade_label_snapshot, term_code_snapshot, catalog_entry_key_snapshot, created_at, updated_at
           ...
           '去重', '自定义', '完成', '[]', g.id, g.current_version_id, 'PRIMARY', 'SIX_THREE', 'G1', '一年级', 'FULL_YEAR', 'PRIMARY_G1',
     ...
     LIMIT 1`;
  await afterSix.query(occSql, [randomUUID(), seriesId]);
  ...
  if (!duplicateBlocked) {
    throw new Error('duplicate occurrence_key was not rejected');
  }
  await afterSix.end();
  if (prisma(['migrate', 'resolve', '--applied', SEVENTH], fixtureUrl) !== 0) process.exit(1);

  recordSixToSevenFixtureUrl(fixtureUrl, process.env);
  if (existsSync(runtimeFile) && shouldWriteLocalRuntime(process.env.STP004_ADMIN_DATABASE_URL, fixtureUrl)) {
    ...
    if (/^STP006_SIX_TO_SEVEN_DATABASE_URL=/m.test(text)) {
      text = text.replace(/^STP006_SIX_TO_SEVEN_DATABASE_URL=.*$/m, `STP006_SIX_TO_SEVEN_DATABASE_URL=${fixtureUrl}`);
    } else {
      text += `STP006_SIX_TO_SEVEN_DATABASE_URL=${fixtureUrl}\n`;
    }
    writeFileSync(runtimeFile, text);
  }
  process.stdout.write(`six→seven fixture ready: ${fixtureName}; catalog preserved; unique key enforced.\n`);
}
