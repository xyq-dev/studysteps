import { existsSync, readFileSync, writeFileSync, appendFileSync } from 'node:fs';
import { dirname, join, resolve } from 'node:path';
import { fileURLToPath } from 'node:url';
import { spawnSync } from 'node:child_process';
import { createRequire } from 'node:module';
import { createHash, randomUUID } from 'node:crypto';

const require = createRequire(join(dirname(fileURLToPath(import.meta.url)), '../apps/api/package.json'));
const pg = require('pg');

const root = join(dirname(fileURLToPath(import.meta.url)), '..');
const runtimeFile = join(root, '.local', 'stp004-pg', 'runtime.env');

const TEN = [
  '20260914000000_stp004_identity_profiles_consents',
  '20260914170000_stp004_step_up_bound_session',
  '20260914233000_stp004_expand_lookup_replacement',
  '20260915160000_stp004_parent_keys_cycle_clock',
  '20260916120000_stp005_grade_catalog_templates',
  '20260916180000_stp005_legacy_fingerprint_and_version_fks',
  '20260917120000_stp006_study_plans_occurrences',
  '20260918090000_stp006_occurrence_version',
  '20260918120000_stp006_series_revisions',
  '20260918180000_stp006_split_parentage',
];
const ELEVENTH = '20260919120000_stp006_task_horizon_jobs';
export const FORBIDDEN = new Set([
  'stp004_identity',
  'stp004_identity_fresh',
  'stp005_four_to_six',
  'stp005_unknown_leftover',
  'stp005_rev_fresh',
  'stp005_rev_four_to_six',
  'stp006_six_to_seven',
  'stp006_seven_to_eight',
  'stp006_eight_to_nine',
  'stp006_nine_to_ten',
  'stp006_fresh',
  'stp006_eleven_to_twelve',
  'stp006_eleven_dirty',
]);
export const FIXTURE_DATABASE = 'stp006_ten_to_eleven';
const originalName = 'stp004_identity';

export function rewriteDb(url, name) {
  return url.replace(/\/[^/?]+(\?|$)/, `/${name}$1`);
}

export function databaseName(url) {
  return new URL(url).pathname.replace(/^\//, '').split('?')[0];
}

export function assertAllowed(name) {
  if (FORBIDDEN.has(name) || !name.startsWith('stp006_')) {
    throw new Error(`refusing database ${name}`);
  }
}

export function recordTenToElevenFixtureUrl(fixtureUrl, env = process.env, io = { appendFileSync }) {
  const name = databaseName(fixtureUrl);
  assertAllowed(name);
  env.STP006_TEN_TO_ELEVEN_DATABASE_URL = fixtureUrl;
  if (env.GITHUB_ENV) {
    io.appendFileSync(env.GITHUB_ENV, `STP006_TEN_TO_ELEVEN_DATABASE_URL=${fixtureUrl}\n`);
  }
}

function loadRuntimeEnv(env = process.env) {
  if (!existsSync(runtimeFile)) return;
  for (const line of readFileSync(runtimeFile, 'utf8').split(/\r?\n/)) {
    if (!line || line.startsWith('#')) continue;
    const index = line.indexOf('=');
    const key = line.slice(0, index);
    if (!env[key]) env[key] = line.slice(index + 1);
  }
}

function sameEndpoint(left, right) {
  const a = new URL(left);
  const b = new URL(right);
  return a.hostname === b.hostname && (a.port || '5432') === (b.port || '5432');
}

export function shouldRequireLocalOriginalIdentity(env = process.env) {
  if (env.CI === 'true' || env.CI === '1') return false;
  if (env.STP004_CI_PREPARED === '1' || env.STP004_CI_PREPARE === '1') return false;
  return true;
}

export function shouldWriteLocalRuntime(localAdminUrl, fixtureUrl) {
  return Boolean(localAdminUrl) && sameEndpoint(localAdminUrl, fixtureUrl);
}

function prisma(args, databaseUrl) {
  const name = databaseName(databaseUrl);
  assertAllowed(name);
  const result = spawnSync('pnpm', ['exec', 'prisma', ...args, '--schema', 'prisma/schema.prisma'], {
    cwd: root,
    env: { ...process.env, DATABASE_URL: databaseUrl },
    encoding: 'utf8',
    windowsHide: true,
    shell: true,
  });
  process.stdout.write(result.stdout || '');
  process.stderr.write(result.stderr || '');
  return result.status ?? 1;
}

async function applySql(client, folder) {
  const sql = readFileSync(join(root, 'prisma/migrations', folder, 'migration.sql'), 'utf8');
  await client.query(sql);
  process.stdout.write(`applied SQL ${folder}\n`);
}

async function currentDb(client) {
  const row = await client.query('SELECT current_database() AS name');
  const name = row.rows[0].name;
  if (FORBIDDEN.has(name)) {
    throw new Error(`query executed on forbidden database ${name}`);
  }
  return name;
}

function digestRows(rows) {
  return createHash('sha256').update(JSON.stringify(rows)).digest('hex');
}

function isDirectRun() {
  const entry = process.argv[1];
  if (!entry) return false;
  return resolve(fileURLToPath(import.meta.url)).toLowerCase() === resolve(entry).toLowerCase();
}

if (isDirectRun()) {
  loadRuntimeEnv();
  const adminUrl = process.env.STP004_ADMIN_DATABASE_URL;
  const apiUrl = process.env.STP004_TEST_DATABASE_URL || process.env.DATABASE_URL;
  if (!adminUrl || !apiUrl) {
    throw new Error('admin or api database URL missing');
  }
  const fixtureName = FIXTURE_DATABASE;
  assertAllowed(fixtureName);
  const postgresAdminUrl = rewriteDb(adminUrl, 'postgres');
  const admin = new pg.Client({ connectionString: postgresAdminUrl, connectionTimeoutMillis: 8000 });
  await admin.connect();
  if (shouldRequireLocalOriginalIdentity()) {
    const originalOpen = await admin.query('SELECT 1 FROM pg_database WHERE datname = $1', [originalName]);
    if (originalOpen.rowCount !== 1) {
      await admin.end();
      throw new Error('original stp004_identity missing; leave it read-only');
    }
  }
  await admin.query(
    `SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE datname = $1 AND pid <> pg_backend_pid()`,
    [fixtureName],
  );
  await admin.query(`DROP DATABASE IF EXISTS ${fixtureName}`);
  await admin.query(`CREATE DATABASE ${fixtureName}`);
  await admin.end();
  const fixtureUrl = rewriteDb(adminUrl, fixtureName);
  const fixture = new pg.Client({ connectionString: fixtureUrl, connectionTimeoutMillis: 8000 });
  await fixture.connect();
  if ((await currentDb(fixture)) !== fixtureName) {
    throw new Error('fixture landed on the wrong database');
  }
  for (const folder of TEN) {
    await applySql(fixture, folder);
  }

  const accountId = randomUUID();
  const studentId = randomUUID();
  const planActive = randomUUID();
  const planPaused = randomUUID();
  const planArchived = randomUUID();
  const seriesDaily = randomUUID();
  const seriesChild = randomUUID();
  const occ = {
    planned: randomUUID(),
    edited: randomUUID(),
    moved: randomUUID(),
    userCancelled: randomUUID(),
    ruleRemoved: randomUUID(),
    splitParent: randomUUID(),
    splitChild: randomUUID(),
    paused: randomUUID(),
    archived: randomUUID(),
    oldGrade: randomUUID(),
  };

  await fixture.query(
    `INSERT INTO accounts (id, status, auth_version, version, created_at, updated_at)
     VALUES ($1, 'ACTIVE', 1, 1, clock_timestamp(), clock_timestamp())`,
    [accountId],
  );
  await fixture.query(
    `INSERT INTO student_profiles (
       id, status, nickname, avatar_preset_id, age_band, age_confirmation_source, age_confirmed_at,
       age_confirmed_by_account_id, timezone, created_by_account_id, version, created_at, updated_at
     ) VALUES (
       $1, 'ONBOARDING', '十到十一基线', 'avatar-03', 'UNDER_14', 'GUARDIAN_DECLARATION', clock_timestamp(),
       $2, 'Asia/Shanghai', $2, 1, clock_timestamp(), clock_timestamp()
     )`,
    [studentId, accountId],
  );
  await fixture.query(
    `INSERT INTO study_plans (id, student_profile_id, status, origin, imported_content_json, timezone_snapshot, version, created_at, updated_at)
     VALUES ($1, $2, 'ACTIVE', 'STUDENT', '[]', 'Asia/Shanghai', 1, clock_timestamp(), clock_timestamp()),
            ($3, $2, 'PAUSED', 'STUDENT', '[]', 'Asia/Shanghai', 1, clock_timestamp(), clock_timestamp()),
            ($4, $2, 'ARCHIVED', 'STUDENT', '[]', 'Asia/Shanghai', 1, clock_timestamp(), clock_timestamp())`,
    [planActive, studentId, planPaused, planArchived],
  );
  await fixture.query(
    `INSERT INTO task_series (
       id, plan_id, name, subject, completion_standard, repeat_kind, start_local_date, ongoing,
       effective_from_local_date, version, created_at, updated_at
     ) VALUES ($1, $2, '补齐基线', '语文', '读完', 'DAILY', '2026-09-10', true, '2026-09-10', 1, clock_timestamp(), clock_timestamp())`,
    [seriesDaily, planActive],
  );
  await fixture.query(
    `INSERT INTO task_series (
       id, plan_id, name, subject, completion_standard, repeat_kind, start_local_date, end_local_date, ongoing,
       effective_from_local_date, effective_to_local_date, version, created_at, updated_at
     ) VALUES ($1, $2, '拆分子', '语文', '后半', 'ONCE', '2026-09-18', '2026-09-18', false, '2026-09-18', '2026-09-18', 1, clock_timestamp(), clock_timestamp())`,
    [seriesChild, planActive],
  );

  async function insertOcc(id, series, key, scheduled, status, name, extra = '') {
    await fixture.query(
      `INSERT INTO task_occurrences (
        id, series_id, occurrence_key, original_local_date, scheduled_local_date, timezone_snapshot, status,
        name_snapshot, subject_snapshot, completion_standard_snapshot, steps_snapshot_json, version,
        grade_config_id, grade_config_version_id, stage_code_snapshot, school_system_code_snapshot,
        grade_code_snapshot, grade_label_snapshot, term_code_snapshot, catalog_entry_key_snapshot, created_at, updated_at
      ) SELECT $1, $2, $3, $3, $4, 'Asia/Shanghai', $5,
               $6, s.subject, s.completion_standard, COALESCE(s.steps_json, '[]'), 1,
               g.id, g.current_version_id, 'PRIMARY', 'SIX_THREE', 'G1', '一年级', 'FULL_YEAR', 'PRIMARY_G1',
               clock_timestamp(), clock_timestamp()
          FROM task_series s
          JOIN grade_configs g ON g.grade_code = 'G1' AND g.stage_code = 'PRIMARY' AND g.school_system_code = 'SIX_THREE'
         WHERE s.id = $2`,
      [id, series, key, scheduled, status, name],
    );
    if (extra) {
      await fixture.query(extra, [id]);
    }
  }

  await insertOcc(occ.planned, seriesDaily, '2026-09-18', '2026-09-18', 'PLANNED', '补齐基线');
  await insertOcc(occ.edited, seriesDaily, '2026-09-19', '2026-09-19', 'PLANNED', '单次改过');
  await insertOcc(occ.moved, seriesDaily, '2026-09-21', '2026-10-01', 'PLANNED', '补齐基线');
  await insertOcc(
    occ.userCancelled,
    seriesDaily,
    '2026-09-22',
    '2026-09-22',
    'CANCELLED',
    '补齐基线',
    `UPDATE task_occurrences SET cancel_reason = 'USER_CANCELLED' WHERE id = $1`,
  );
  await insertOcc(
    occ.ruleRemoved,
    seriesDaily,
    '2026-09-23',
    '2026-09-23',
    'CANCELLED',
    '补齐基线',
    `UPDATE task_occurrences SET cancel_reason = 'SERIES_RULE_REMOVED' WHERE id = $1`,
  );
  await insertOcc(
    occ.splitParent,
    seriesDaily,
    '2026-09-24',
    '2026-09-24',
    'CANCELLED',
    '补齐基线',
    `UPDATE task_occurrences SET cancel_reason = 'SPLIT' WHERE id = $1`,
  );
  await insertOcc(occ.splitChild, seriesChild, '2026-09-24', '2026-09-24', 'PLANNED', '拆分子');
  await fixture.query(`UPDATE task_occurrences SET source_occurrence_id = $1 WHERE id = $2`, [
    occ.splitParent,
    occ.splitChild,
  ]);
  await insertOcc(
    occ.oldGrade,
    seriesDaily,
    '2026-09-11',
    '2026-09-11',
    'COMPLETED',
    '旧年级',
    `UPDATE task_occurrences SET grade_label_snapshot = '二年级', catalog_entry_key_snapshot = 'PRIMARY_G2' WHERE id = $1`,
  );

  const pausedSeries = randomUUID();
  const archivedSeries = randomUUID();
  await fixture.query(
    `INSERT INTO task_series (
       id, plan_id, name, subject, completion_standard, repeat_kind, start_local_date, ongoing,
       effective_from_local_date, version, created_at, updated_at
     ) VALUES ($1, $2, '暂停规则', '数学', '完成', 'DAILY', '2026-09-10', true, '2026-09-10', 1, clock_timestamp(), clock_timestamp())`,
    [pausedSeries, planPaused],
  );
  await fixture.query(
    `INSERT INTO task_series (
       id, plan_id, name, subject, completion_standard, repeat_kind, start_local_date, ongoing,
       effective_from_local_date, version, created_at, updated_at
     ) VALUES ($1, $2, '归档规则', '英语', '完成', 'DAILY', '2026-09-10', true, '2026-09-10', 1, clock_timestamp(), clock_timestamp())`,
    [archivedSeries, planArchived],
  );
  await insertOcc(occ.paused, pausedSeries, '2026-09-18', '2026-09-18', 'PLANNED', '暂停规则');
  await insertOcc(occ.archived, archivedSeries, '2026-09-18', '2026-09-18', 'PLANNED', '归档规则');

  const editAdj = randomUUID();
  const moveAdj = randomUUID();
  const contentRevAdj = randomUUID();
  const scheduleRevAdj = randomUUID();
  await fixture.query(
    `INSERT INTO plan_adjustments (id, plan_id, series_id, occurrence_id, reason_code, payload_json, created_at)
     VALUES ($1, $2, $3, $4, 'TASK_CONTENT_EDITED', '{}', clock_timestamp()),
            ($5, $2, $3, $6, 'TASK_RESCHEDULED', '{}', clock_timestamp()),
            ($7, $2, $3, $9, 'SERIES_FUTURE_CONTENT_CHANGED', '{}', clock_timestamp()),
            ($8, $2, $3, $9, 'SERIES_FUTURE_SCHEDULE_CHANGED', '{}', clock_timestamp())`,
    [editAdj, planActive, seriesDaily, occ.edited, moveAdj, occ.moved, contentRevAdj, scheduleRevAdj, occ.planned],
  );
  await fixture.query(`UPDATE task_occurrences SET content_exception_adjustment_id = $1 WHERE id = $2`, [
    editAdj,
    occ.edited,
  ]);
  await fixture.query(`UPDATE task_occurrences SET schedule_exception_adjustment_id = $1 WHERE id = $2`, [
    moveAdj,
    occ.moved,
  ]);
  await fixture.query('BEGIN');
  await fixture.query(
    `INSERT INTO task_series_revisions (
       task_series_id, revision_no, change_kind, effective_from_occurrence_key,
       name, subject, completion_standard, steps_json, source_adjustment_id, created_at
     ) VALUES ($1, 2, 'CONTENT', '2026-09-18', '未来内容', '语文', '读完', '[]', $2, clock_timestamp())`,
    [seriesDaily, contentRevAdj],
  );
  await fixture.query(
    `INSERT INTO task_series_revisions (
       task_series_id, revision_no, change_kind, effective_from_occurrence_key, repeat_kind, ongoing, source_adjustment_id, created_at
     ) VALUES ($1, 3, 'SCHEDULE', '2026-09-18', 'DAILY', true, $2, clock_timestamp())`,
    [seriesDaily, scheduleRevAdj],
  );
  await fixture.query(`UPDATE task_series SET version = 3 WHERE id = $1`, [seriesDaily]);
  await fixture.query('COMMIT');

  const beforeOcc = await fixture.query(
    `SELECT id, series_id, occurrence_key, original_local_date, scheduled_local_date, status, cancel_reason,
            name_snapshot, grade_label_snapshot, source_occurrence_id,
            content_exception_adjustment_id, schedule_exception_adjustment_id
       FROM task_occurrences ORDER BY id`,
  );
  const beforeAdj = await fixture.query(
    `SELECT id, plan_id, series_id, occurrence_id, reason_code FROM plan_adjustments ORDER BY id`,
  );
  const beforeRev = await fixture.query(
    `SELECT task_series_id, revision_no, change_kind, name FROM task_series_revisions ORDER BY task_series_id, revision_no`,
  );
  const beforePlans = await fixture.query(`SELECT id, status, version FROM study_plans ORDER BY id`);
  const beforeDigest = digestRows({
    occ: beforeOcc.rows,
    adj: beforeAdj.rows,
    rev: beforeRev.rows,
    plans: beforePlans.rows,
  });
  await fixture.end();

  for (const folder of TEN) {
    if (prisma(['migrate', 'resolve', '--applied', folder], fixtureUrl) !== 0) process.exit(1);
  }
  const afterTen = new pg.Client({ connectionString: fixtureUrl, connectionTimeoutMillis: 8000 });
  await afterTen.connect();
  if (await afterTen.query(`SELECT to_regclass('task_horizon_jobs') AS jobs`).then((r) => r.rows[0].jobs)) {
    throw new Error('job table must not exist before eleventh');
  }
  await applySql(afterTen, ELEVENTH);
  await afterTen.end();
  if (prisma(['migrate', 'resolve', '--applied', ELEVENTH], fixtureUrl) !== 0) process.exit(1);

  const after = new pg.Client({ connectionString: fixtureUrl, connectionTimeoutMillis: 8000 });
  await after.connect();
  const applied = await after.query(`
    SELECT COUNT(*)::int AS n FROM _prisma_migrations
     WHERE finished_at IS NOT NULL AND rolled_back_at IS NULL
  `);
  if (applied.rows[0].n !== 11) {
    throw new Error(`expected 11 applied migrations, found ${applied.rows[0].n}`);
  }
  const afterOcc = await after.query(
    `SELECT id, series_id, occurrence_key, original_local_date, scheduled_local_date, status, cancel_reason,
            name_snapshot, grade_label_snapshot, source_occurrence_id,
            content_exception_adjustment_id, schedule_exception_adjustment_id
       FROM task_occurrences ORDER BY id`,
  );
  const afterAdj = await after.query(
    `SELECT id, plan_id, series_id, occurrence_id, reason_code FROM plan_adjustments ORDER BY id`,
  );
  const afterRev = await after.query(
    `SELECT task_series_id, revision_no, change_kind, name FROM task_series_revisions ORDER BY task_series_id, revision_no`,
  );
  const afterPlans = await after.query(`SELECT id, status, version FROM study_plans ORDER BY id`);
  if (digestRows({ occ: afterOcc.rows, adj: afterAdj.rows, rev: afterRev.rows, plans: afterPlans.rows }) !== beforeDigest) {
    throw new Error('ten-to-eleven changed existing business digest');
  }
  const jobs = await after.query(
    `SELECT plan_id, state, state_reason FROM task_horizon_jobs ORDER BY plan_id`,
  );
  if (jobs.rowCount !== 3) {
    throw new Error(`expected 3 standing jobs, found ${jobs.rowCount}`);
  }
  const byPlan = Object.fromEntries(jobs.rows.map((row) => [row.plan_id, row]));
  if (byPlan[planActive]?.state !== 'READY' || byPlan[planActive]?.state_reason != null) {
    throw new Error('ACTIVE plan job was not READY');
  }
  if (byPlan[planPaused]?.state !== 'BLOCKED' || byPlan[planPaused]?.state_reason !== 'PLAN_PAUSED') {
    throw new Error('PAUSED plan job was not BLOCKED/PLAN_PAUSED');
  }
  if (byPlan[planArchived]?.state !== 'RETIRED' || byPlan[planArchived]?.state_reason !== 'PLAN_ARCHIVED') {
    throw new Error('ARCHIVED plan job was not RETIRED/PLAN_ARCHIVED');
  }

  async function expectReject(label, work) {
    let blocked = false;
    try {
      await work();
    } catch {
      blocked = true;
    }
    if (!blocked) {
      throw new Error(`${label} was not rejected`);
    }
  }

  await expectReject('second job same plan', () =>
    after.query(
      `INSERT INTO task_horizon_jobs (plan_id, state, available_at, requested_generation, processed_generation, attempt_count, last_inserted_count, created_at, updated_at)
       VALUES ($1, 'READY', clock_timestamp(), 1, 0, 0, 0, clock_timestamp(), clock_timestamp())`,
      [planActive],
    ),
  );
  await expectReject('missing plan fk', () =>
    after.query(
      `INSERT INTO task_horizon_jobs (plan_id, state, available_at, requested_generation, processed_generation, attempt_count, last_inserted_count, created_at, updated_at)
       VALUES ($1, 'READY', clock_timestamp(), 1, 0, 0, 0, clock_timestamp(), clock_timestamp())`,
      [randomUUID()],
    ),
  );
  await expectReject('bad state', () =>
    after.query(`UPDATE task_horizon_jobs SET state = 'QUEUED' WHERE plan_id = $1`, [planActive]),
  );
  await expectReject('leased without token', () =>
    after.query(`UPDATE task_horizon_jobs SET state = 'LEASED', available_at = NULL WHERE plan_id = $1`, [planActive]),
  );
  await expectReject('ready without available', () =>
    after.query(`UPDATE task_horizon_jobs SET available_at = NULL WHERE plan_id = $1`, [planActive]),
  );
  await expectReject('blocked bad reason', () =>
    after.query(
      `UPDATE task_horizon_jobs SET state = 'BLOCKED', state_reason = 'SESSION_EXPIRED', available_at = clock_timestamp() WHERE plan_id = $1`,
      [planPaused],
    ),
  );

  const insertPlan = randomUUID();
  await after.query(
    `INSERT INTO study_plans (id, student_profile_id, status, origin, imported_content_json, timezone_snapshot, version, created_at, updated_at)
     VALUES ($1, $2, 'ACTIVE', 'STUDENT', '[]', 'Asia/Shanghai', 1, clock_timestamp(), clock_timestamp())`,
    [insertPlan, studentId],
  );
  const triggered = await after.query(`SELECT state FROM task_horizon_jobs WHERE plan_id = $1`, [insertPlan]);
  if (triggered.rows[0]?.state !== 'READY') {
    throw new Error('insert trigger did not create READY job');
  }
  await after.end();

  recordTenToElevenFixtureUrl(fixtureUrl, process.env);
  if (existsSync(runtimeFile) && shouldWriteLocalRuntime(process.env.STP004_ADMIN_DATABASE_URL, fixtureUrl)) {
    let text = readFileSync(runtimeFile, 'utf8');
    if (/^STP006_TEN_TO_ELEVEN_DATABASE_URL=/m.test(text)) {
      text = text.replace(/^STP006_TEN_TO_ELEVEN_DATABASE_URL=.*$/m, `STP006_TEN_TO_ELEVEN_DATABASE_URL=${fixtureUrl}`);
    } else {
      text += `STP006_TEN_TO_ELEVEN_DATABASE_URL=${fixtureUrl}\n`;
    }
    writeFileSync(runtimeFile, text);
  }
  process.stdout.write(`ten→eleven fixture ready: ${fixtureName}; standing-job constraints enforced.\n`);
}
